 ...
 *
 * ⚠️ NOTE: appConfig and retryFetch are loaded from:
...
// ==============================================================
// 0. SADHANA CATALOG EXTENSION — appConfig is already loaded by appConfig.js
// ==============================================================
...
window.escapeHtmlUtil = window.escapeHtml;

// retryFetch — defined here as fallback if retryHelper.js is not loaded
if (typeof window.retryFetch !== 'function') {
  window.retryFetch = async function retryFetch(input, init = {}, maxRetry = 3) {
    let attempt = 0;
    while (true) {
      try {
...
var retryFetch = window.retryFetch;
