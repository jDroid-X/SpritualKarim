...
const DemoAuth = {
  ...
  /**
   * Resolve a member profile by <first>.<last> name or aliases
   */
  _resolveProfileByUsername(username) {
    if (!username) return null;
    const u = username.toLowerCase().trim();

    // Canonical seed catalog matching the 4-tier hierarchy
    const seedProfiles = [
      { id: "prof-admin-01", name: "Spiritual Karim Khan", username: "karim.khan", aliases: ["admin", "karim", "karimkhan", "spiritualkarim", "karim.khan", "spiritual karim khan"], role: "master", defaultPass: "admin123" },
      { id: "prof-healer-02", name: "Acharya Devendra", username: "acharya.devendra", aliases: ["acharya.devendra", "devendra.acharya", "devendra", "healer"], role: "healer", defaultPass: "healer123" },
      { id: "prof-healer-03", name: "Maa Anandita Devi", username: "anandita.devi", aliases: ["anandita.devi", "maa.anandita", "anandita"], role: "healer", defaultPass: "healer123" },
      { id: "prof-trainee-04", name: "Amitabh Sen", username: "amitabh.sen", aliases: ["amitabh.sen", "amitabh", "trainee"], role: "trainee", defaultPass: "trainee123" },
      { id: "prof-trainee-05", name: "Rajeshwari Joshi", username: "rajeshwari.joshi", aliases: ["rajeshwari.joshi", "rajeshwari"], role: "trainee", defaultPass: "trainee123" },
      { id: "prof-trainee-06", name: "Rohan Verma", username: "rohan.verma", aliases: ["rohan.verma", "rohan"], role: "trainee", defaultPass: "trainee123" },
      { id: "prof-trainee-07", name: "Priya Sharma", username: "priya.sharma", aliases: ["priya.sharma", "priya"], role: "trainee", defaultPass: "trainee123" },
      { id: "prof-devotee-08", name: "Sunita Mehra", username: "sunita.mehra", aliases: ["sunita.mehra", "sunita", "devotee"], role: "devotee", defaultPass: "devotee123" },
      { id: "prof-devotee-09", name: "Deepak Saxena", username: "deepak.saxena", aliases: ["deepak.saxena", "deepak", "seeker"], role: "devotee", defaultPass: "devotee123" },
      { id: "prof-devotee-10", name: "Aarohi Patel", username: "aarohi.patel", aliases: ["aarohi.patel", "aarohi"], role: "devotee", defaultPass: "devotee123" },
      { id: "prof-devotee-11", name: "Meera Nair", username: "meera.nair", aliases: ["meera.nair", "meera"], role: "devotee", defaultPass: "devotee123" },
      { id: "prof-devotee-12", name: "Suresh Rao", username: "suresh.rao", aliases: ["suresh.rao", "suresh"], role: "devotee", defaultPass: "devotee123" }
    ];

    const fromSeed = seedProfiles.find(s => s.username === u || (s.aliases && s.aliases.includes(u)));
    if (fromSeed) return fromSeed;

    // Search dynamically from storage
    try {
      let profiles = [];
      const raw = localStorage.getItem("sk_admin_profiles_v3") || localStorage.getItem("sk_profiles");
      ...
      for (const p of profiles) {
        ...
        if (dotName === u || pName.includes(u) || (p.email && p.email.toLowerCase().includes(u))) {
          let role = (p.profileType || "devotee").toLowerCase();
          if (role === "seeker") role = "devotee";
          const defaultPass = role === "master" ? "admin123" : role === "healer" ? "healer123" : role === "trainee" ? "trainee123" : "devotee123";
          return { id: p.id, name: p.name, username: dotName, role, defaultPass };
        }
      }
    } catch (e) {}
  ...
  /**
   * Login with <first>.<last> username and default password
   */
  login(username, password) {
    try {
      const uKey = (username || '').toLowerCase().trim();
      const profile = this._resolveProfileByUsername(uKey);
      ...
      if (profile) {
        ...
        if (validPasswords.includes(password) || password === profile.username) {
          const session = {
            username: profile.username || uKey,
            role: profile.role,
          ...
          sessionStorage.setItem('sk_active_profile_id', profile.id);
          localStorage.setItem('sk_active_profile_id', profile.id);
          return { success: true, session, profile };
        } else {
          return { success: false, reason: 'Incorrect password. Default for ' + profile.role + ' is ' + profile.defaultPass };
        }
      }

      // Check custom registered users
      const users = JSON.parse(localStorage.getItem('sk_auth_users') || '{}');
      const user = users[uKey] || users[username];
      if (!user) return { success: false, reason: 'User not found. Enter your <first>.<last> name (e.g. amitabh.sen, sunita.mehra, karim.khan).' };
      const hash = btoa(password + '_demo_salt_2026');
      if (user.hash !== hash) return { success: false, reason: 'Incorrect password.' };
      const session = { username: uKey, role: user.role, loginAt: Date.now() };
      sessionStorage.setItem(this._key, JSON.stringify(session));
