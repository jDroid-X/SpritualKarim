    ...
    sadhanaPreviewContainer.addEventListener("click", (e) => {
      ...
      if (btnApply) {
        ...
        if (confirm(`Apply for Sacred Sadhana Initiation into "${catalogItem.title}"?\n\nThis sends an approval request to your Mentor/Healer and initiates your upgrade to Trainee Sadhak upon approval.`)) {
          const defaultMentor = (typeof appConfig !== "undefined" && appConfig.defaultMentorCode) ||
  ...
  if (this.view.tierPanelProfilesList) {
    this.view.tierPanelProfilesList.addEventListener("click", (e) => {
      ...
      if (card) {
        ...
        this.view.currentSelectedTierMember = this.model.getActiveProfile();
        this._renderCurrentState();
        ...
        const rightPanel = document.getElementById("tier-right-profile-details");
        const activeP = this.model.getActiveProfile();
        if (rightPanel && activeP) {
          const initials = (activeP.name || "SK")
            .split(" ")
            .map((w) => w[0])
            .slice(0, 2)
            .join("")
            .toUpperCase();
          const roleType = activeP.profileType || "DEVOTEE";
          const roleColors = {
            ADMIN: "#8b5cf6",
            HEALER: "#10b981",
            TRAINEE: "#f59e0b",
          ...
          const roleColor = roleColors[roleType] || "#3b82f6";
          const isPaid = activeP.isPaid !== false && activeP.paymentStatus !== "FREE";

          rightPanel.innerHTML = `
            <div class="tier-detail-card">
              <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                <div class="avatar-circle" style="width: 56px; height: 56px; font-size: 1.25rem; border: 2.5px solid ${roleColor}; background: linear-gradient(135deg, #1e1b4b, #312e81); color: #fff; display: flex; align-items: center; justify-content: center; border-radius: 50%; font-weight: 700; box-shadow: 0 4px 14px rgba(0,0,0,0.4);">${initials}</div>
                <div>
                  <h3 style="margin: 0 0 0.25rem 0; font-size: 1.15rem; color: var(--text-primary, #fff); font-weight: 700;">${activeP.name || "Unknown"}</h3>
                  <div style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
                    <span class="role-badge" style="background: ${roleColor}; font-size: 0.68rem; padding: 0.1rem 0.45rem;">${roleType} • LEVEL ${activeP.level || 1}</span>
                    <span class="stamp-indicator ${isPaid ? "stamp-paid" : "stamp-free"}" style="font-size: 0.65rem;">${isPaid ? "PAID" : "FREE"}</span>
                    <span class="status-pill ${activeP.isActive ? "active" : ""}" style="font-size: 0.65rem;">${activeP.isActive ? "Active" : "Inactive"}</span>
                  </div>
                </div>
              </div>

              <div class="tier-detail-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.65rem;">
                <div class="tier-detail-item" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 0.5rem; padding: 0.55rem 0.75rem;">
                  <span style="font-size: 0.68rem; color: var(--text-muted); display: block; margin-bottom: 0.15rem;">📋 Reference Code</span>
                  <span style="font-size: 0.82rem; font-weight: 600; font-family: monospace; color: var(--gold-400, #d4af37);">${activeP.referenceCode || "—"}</span>
                </div>
                <div class="tier-detail-item" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 0.5rem; padding: 0.55rem 0.75rem;">
                  <span style="font-size: 0.68rem; color: var(--text-muted); display: block; margin-bottom: 0.15rem;">📅 Date of Joining</span>
                  <span style="font-size: 0.82rem; font-weight: 600; color: var(--text-primary, #fff);">${activeP.dateOfJoining || activeP.joinDate || "—"}</span>
                </div>
                <div class="tier-detail-item" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 0.5rem; padding: 0.55rem 0.75rem;">
                  <span style="font-size: 0.68rem; color: var(--text-muted); display: block; margin-bottom: 0.15rem;">📞 Contact</span>
                  <span style="font-size: 0.82rem; font-weight: 600; color: var(--text-primary, #fff);">${activeP.mobile || activeP.phone || "—"}</span>
                </div>
                <div class="tier-detail-item" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 0.5rem; padding: 0.55rem 0.75rem;">
                  <span style="font-size: 0.68rem; color: var(--text-muted); display: block; margin-bottom: 0.15rem;">🛡️ Sponsor</span>
                  <span style="font-size: 0.82rem; font-weight: 600; color: var(--text-primary, #fff);">${activeP.referredByCode || activeP.sponsorCode || "ROOT"}</span>
                </div>
                <div class="tier-detail-item" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 0.5rem; padding: 0.55rem 0.75rem;">
                  <span style="font-size: 0.68rem; color: var(--text-muted); display: block; margin-bottom: 0.15rem;">📧 Email</span>
                  <span style="font-size: 0.82rem; font-weight: 600; color: var(--text-primary, #fff);">${activeP.email || "—"}</span>
                </div>
                <div class="tier-detail-item" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 0.5rem; padding: 0.55rem 0.75rem;">
                  <span style="font-size: 0.68rem; color: var(--text-muted); display: block; margin-bottom: 0.15rem;">📍 City / Location</span>
                  <span style="font-size: 0.82rem; font-weight: 600; color: var(--text-primary, #fff);">${activeP.city || activeP.location || "—"}</span>
                </div>
              </div>
            </div>
          `;
    ...
    container.querySelectorAll(".sadhana-progress-card").forEach((card) => {
      ...
      if (title) {
        items.push({
          ...
          currentStreak,
          diaryNotes,
          mentorCode: active.referredByCode || (typeof appConfig !== "undefined" && appConfig.defaultMentorCode) || this.model.settings.defaultMentorCode || "SKHM-ADM1-7788-9900",
        });
      }
    });
    return items;
