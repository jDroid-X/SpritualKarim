class ProfileController {
  ...
  _bindSidebarNavigationAndTierEvents() {
    ...
    // 2. App Hierarchy Tiers in Sidebar (Tree Explorer Nodes)
    document
      ...
      .forEach((item) => {
        item.addEventListener("click", (e) => {
          ...
          if (tier === 1) {
            ...
            this.view.showSlideToast(
              "Admin Master Tier",
              "👑 Viewing Founder Master Profiles & Governance Controls",
              "info",
              2500,
            );
          } else if (tier === 2) {
            ...
            this.view.showSlideToast(
              "Healer Connect Tier",
              isMasterLoggedIn
                ? "👑 [Master Control] Viewing Certified Healers Tier Details"
            ...
            this.view.showSlideToast(
              "Trainee Sadhak Tier",
              isMasterLoggedIn
                ? "👑 [Master Control] Viewing Trainee Mentorship Tier Details"
            ...
            this.view.showSlideToast(
              "Devotee Seeker Tier",
              isMasterLoggedIn
                ? "👑 [Master Control] Viewing Devotee & Seeker Tier Details"
          ...
          // Also open flyout panel with detailed profile cards
  ...
  _bindEvents() {
    ...
    if (btnSaveAuthMatrix) {
      btnSaveAuthMatrix.addEventListener("click", (e) => {
        ...
        this.view.showSlideToast(
          "Matrix Updated",
          "🛡️ Authorization Matrix saved and applied in real-time across all portals!",
          "success",
          3000,
        );
      });
    ...
    document.addEventListener("click", (e) => {
      ...
      if (e.target.closest("#pending-approval-notification-banner")) {
        this.view.openPendingApprovalsDrawer(this.model.getPairingInvites());
        return;
      }
      ...
      if (btnDrawerApprove) {
        ...
        if (id) {
          const roleSelect = document.getElementById(`drawer-select-role-${id}`) || document.querySelector(`.select-pairing-role[data-invite-id="${id}"]`);
    ...
    if (this.view.btnSidebarLogout) {
      this.view.btnSidebarLogout.addEventListener("click", (e) => {
        ...
        const targetUrl = isInSubfolder ? "../logout.html" : "logout.html";
    ...
    // Admin Master Live Hierarchy Metrics Tiles Navigation (Slide Down Panel - Robust Delegated)
    document.addEventListener("click", (e) => {
