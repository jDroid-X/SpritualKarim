...
class ProfileView {
  ...
  renderAdminMetricsTiles(metrics, roleMode = "MASTER") {
    ...
    if (this.adminMetricsStrip) {
      const isAdmin = roleMode === "MASTER" || roleMode === "ADMIN";
    ...
    if (mNew) mNew.textContent = `👑 Founder Master`;
    ...
    if (hNew) {
      ...
      hNew.textContent = `✨ New Joined: ${newCount}`;
    }
    ...
    if (tNew) {
      ...
      tNew.textContent = `✨ New Joined: ${newCount}`;
    }
    ...
    if (dNew) {
      ...
      dNew.textContent = `✨ New Joined: ${newCount}`;
    }
  ...
  openPendingApprovalsDrawer(invites = [], selectedInviteId = null) {
    ...
    if (body) {
      if (!invites || invites.length === 0) {
        ...
        // 1. SORT: Pending accounts ALWAYS on top, followed by approved/rejected (sorted by newest timestamp)
        ...
        // Stats Metadata Header (Pending, Approved, Rejected, Expired, Total)
        ...
        const expiredInvites = sortedInvites.filter(i => {
          const remMs = (i.expiresAtMs || ((i.createdAtMs || i.timestamp || Date.now()) + inviteTtl)) - now;
          return (i.status === "PENDING" || i.status === "PENDING_APPROVAL") && remMs <= 0;
        });
        const expiredCount = expiredInvites.length;

        const approvedCount = sortedInvites.filter(i => i.status === "APPROVED").length;
        const rejectedCount = sortedInvites.filter(i => i.status === "REJECTED" || i.status === "REVISION").length;

        // Update footer counters if present
        const footerCountEl = document.getElementById("pending-drawer-count");
        if (footerCountEl) {
          footerCountEl.textContent = `${pendingCount} Pending / ${expiredCount} Expired / ${totalCount} Total`;
        }

        let activeInv = sortedInvites.find(i => i.id === selectedInviteId);
        if (!activeInv) {
          activeInv = sortedInvites.find(i => i.status === "PENDING" || i.status === "PENDING_APPROVAL") || sortedInvites[0];
        }

        const inv = activeInv;
        const remainingMs = (inv.expiresAtMs || ((inv.createdAtMs || inv.timestamp || Date.now()) + inviteTtl)) - now;
        const isExpired = (inv.status === "PENDING" || inv.status === "PENDING_APPROVAL") && remainingMs <= 0;
        ...
        const defaultRole = inv.assignedRole || (inv.type === "HEALER_APPLICATION" ? "HEALER" : inv.type === "SADHANA_APPLICATION" ? "TRAINEE" : "DEVOTEE");
        ...
        try {
          ...
          idProofDoc = docs.find(d => (d.profileId === inv.id || d.profileId === inv.devoteeCode) && d.docType === 'id_proof') || null;
          addrProofDoc = docs.find(d => (d.profileId === inv.id || d.profileId === inv.devoteeCode) && d.docType === 'address_proof') || null;
          photoDoc = docs.find(d => (d.profileId === inv.id || d.profileId === inv.devoteeCode) && d.docType === 'photo') || null;
        } catch(e) {}

        const photoSrc = photoDoc?.dataUrl || inv.documentsUploaded?.photo || "";
        const idProofAvailable = Boolean(idProofDoc || inv.documentsUploaded?.idProof || inv.documents?.idProof);
        const addrProofAvailable = Boolean(addrProofDoc || inv.documentsUploaded?.addressProof || inv.documents?.addressProof);
        const photoAvailable = Boolean(photoDoc || inv.documentsUploaded?.photo || inv.documents?.photo);

        const baseUrl = window.location.origin + window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/') + 1);
        const fullJoinUrl = `${baseUrl}join.html?ref=${inv.sponsorCode || ((this.model && typeof this.model.getDefaultMentorCode === 'function') ? this.model.getDefaultMentorCode() : 'SKHM-ADM1-7788-9900')}&pin=${inv.activationPin || '140610'}`;
        const fullDevoteeUrl = `${baseUrl}Devotee/index.html?profileId=${inv.devoteeCode || inv.id}`;

        body.innerHTML = `
          <!-- STATS METADATA BAR -->
          <div style="margin-bottom: 0.85rem; padding: 0.65rem 0.85rem; background: var(--bg-card, rgba(0,0,0,0.3)); border: 1px solid var(--border-subtle, rgba(212,175,55,0.25)); border-radius: 8px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 0.4rem;">
            <div style="display: flex; align-items: center; gap: 0.35rem; font-size: 0.74rem; font-weight: 700; color: var(--text-primary); flex-wrap: wrap;">
              <span>📊 Metadata:</span>
              <span style="background: rgba(245,158,11,0.2); border: 1px solid #f59e0b; color: #f59e0b; padding: 0.12rem 0.45rem; border-radius: 12px;">⏳ Pending: ${pendingCount}</span>
              <span style="background: rgba(16,185,129,0.2); border: 1px solid #10b981; color: #10b981; padding: 0.12rem 0.45rem; border-radius: 12px;">✓ Approved: ${approvedCount}</span>
              <span style="background: rgba(239,68,68,0.2); border: 1px solid #ef4444; color: #ef4444; padding: 0.12rem 0.45rem; border-radius: 12px;">✕ Rejected: ${rejectedCount}</span>
              <span style="background: rgba(244,63,94,0.2); border: 1px solid #f43f5e; color: #f43f5e; padding: 0.12rem 0.45rem; border-radius: 12px;">⌛ Expired: ${expiredCount}</span>
              <span style="background: rgba(212,175,55,0.2); border: 1px solid var(--gold-400); color: var(--gold-400); padding: 0.12rem 0.45rem; border-radius: 12px;">Total: ${totalCount}</span>
            </div>
            <span style="font-size: 0.68rem; color: var(--text-muted); font-weight: 600;">Sorted: Pending First ⬇</span>
          </div>

          <!-- 2-PANEL SPLIT VIEW CONTAINER -->
          <div class="drawer-split-container" style="display: flex; gap: 1rem; height: calc(100% - 48px); min-height: 520px; align-items: stretch;">
            
            <!-- LEFT PANEL: APPLICANT LIST SIDEBAR -->
            <div class="drawer-split-left-pane" style="width: 210px; flex-shrink: 0; background: var(--bg-card, rgba(0,0,0,0.25)); border: 1px solid var(--border-subtle, rgba(255,255,255,0.1)); border-radius: 8px; padding: 0.6rem; overflow-y: auto; display: flex; flex-direction: column; gap: 0.45rem;">
              <div style="font-size: 0.7rem; font-weight: 800; color: var(--gold-400); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.25rem; padding: 0.2rem 0.4rem;">
                📋 Applicants (${sortedInvites.length})
              </div>
              ${sortedInvites.map(item => {
                const isSelected = item.id === activeInv.id;
                const isItemApproved = item.status === "APPROVED";
                const isItemPending = item.status === "PENDING" || item.status === "PENDING_APPROVAL";
                const statusColor = isItemApproved ? "#10b981" : isItemPending ? "#f59e0b" : "#ef4444";
                const itemInitials = item.seekerName ? item.seekerName.split(' ').map(n=>n[0]).slice(0,2).join('').toUpperCase() : "SK";
                
                return `
                  <div class="applicant-tab-chip drawer-applicant-card-item ${isSelected ? 'active' : ''}" data-invite-id="${item.id}" style="padding: 0.55rem 0.65rem; border-radius: 8px; border: 1.5px solid ${isSelected ? 'var(--gold-400)' : 'var(--border-subtle, rgba(255,255,255,0.08))'}; background: ${isSelected ? 'rgba(212,175,55,0.18)' : 'var(--bg-card, rgba(0,0,0,0.2))'}; cursor: pointer; transition: all 0.18s ease; display: flex; align-items: center; gap: 0.5rem;">
                    <div style="width: 28px; height: 28px; border-radius: 50%; background: ${statusColor}22; border: 1.5px solid ${statusColor}; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 800; color: ${statusColor}; flex-shrink: 0;">
                      ${itemInitials}
                    </div>
                    <div style="min-width: 0; flex: 1;">
                      <div style="font-size: 0.78rem; font-weight: 700; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                        ${item.seekerName || 'Devotee Seeker'}
                      </div>
                      <div style="font-size: 0.65rem; color: ${statusColor}; font-weight: 700;">
                        ${isItemApproved ? '🟢 Approved' : '⏳ Pending'}
                      </div>
                    </div>
                  </div>
                `;
            ...
            <!-- RIGHT PANEL: DETAILED APPLICANT DOSSIER & ACTIONS -->
            <div class="drawer-split-right-pane" style="flex: 1; min-width: 0; overflow-y: auto; padding-right: 0.2rem;">
              
              <!-- HERO CARD -->
              <div class="dossier-box" style="background: linear-gradient(135deg, rgba(212,175,55,0.15) 0%, rgba(16,185,129,0.1) 100%); border: 1.5px solid var(--gold-400); border-radius: 10px; padding: 1.1rem; margin-bottom: 0.85rem;">
                <div style="display: flex; gap: 0.9rem; align-items: center;">
                  <div class="drawer-applicant-avatar btn-preview-document-image" data-doc-title="${inv.seekerName || 'Applicant'} Photo" data-doc-type="photo" data-invite-id="${inv.id}" style="width: 58px; height: 58px; border-radius: 50%; background: radial-gradient(circle, rgba(212,175,55,0.35) 0%, rgba(16,185,129,0.2) 100%); border: 2.5px solid var(--gold-400); display: flex; align-items: center; justify-content: center; font-size: 1.25rem; font-weight: 800; color: var(--gold-400); cursor: pointer; flex-shrink: 0;" title="Click to view full photo">
                    ${photoSrc ? `<img src="${photoSrc}" alt="Seeker" style="width:100%; height:100%; object-fit:cover; border-radius:50%;">` : initials}
                  </div>
                  <div style="flex: 1; min-width: 0;">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 0.25rem;">
                      <h3 style="margin: 0; font-size: 1.15rem; color: var(--text-primary); font-family: var(--font-heading); font-weight: 800;">${inv.seekerName || "Devotee Applicant"}</h3>
                    ...
                    <div style="font-family: var(--font-mono); font-size: 0.85rem; color: var(--gold-400); font-weight: 800; margin-top: 0.15rem;">
                      ${inv.devoteeCode || inv.hardwareNonce || "SKDV-PROVISIONED"}
                    </div>
                    <div style="font-size: 0.73rem; color: var(--text-muted); margin-top: 0.2rem;">
                      Pairing PIN: <strong style="color:#10b981; font-family: var(--font-mono);">${inv.activationPin || "140610"}</strong> • 24h Window: <span style="color:#f59e0b; font-weight:700;">${isApproved ? 'Linked' : `${hours}h ${mins}m remaining`}</span>
                    </div>
                  </div>
                </div>
              </div>

              <!-- FULL HYPERLINK TESTING PANEL (ADMIN DIRECT ACCESS) -->
              <div class="dossier-box" style="background: rgba(56, 189, 248, 0.08); border: 1.5px solid #38bdf8; border-radius: 8px; padding: 0.8rem 0.95rem; margin-bottom: 0.85rem;">
                <div style="font-size: 0.72rem; font-weight: 800; color: #38bdf8; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.4rem; display: flex; align-items: center; gap: 0.35rem;">
                  <span>🌐</span> Direct Testing Hyperlinks (Full URL Access)
                </div>
                <div style="display: flex; flex-direction: column; gap: 0.35rem; font-size: 0.75rem;">
                  ...
                  <div>
                    <span style="color: var(--text-muted); font-weight: 600;">Dedicated Devotee Website:</span><br>
              ...
              <div class="dossier-box" style="background: var(--bg-card, rgba(0,0,0,0.25)); border: 1px solid var(--border-subtle, rgba(255,255,255,0.08)); border-radius: 8px; padding: 0.85rem 0.95rem; margin-bottom: 0.85rem;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.55rem; font-size: 0.78rem;">
                  <div>
                    <span style="color: var(--text-muted);">Phone Number:</span><br>
                    <strong style="color: var(--text-primary); font-family: var(--font-mono);">${inv.seekerPhone || "—"}</strong>
                  ...
                  <div>
                    ...
                    <strong style="color: var(--text-primary);">${inv.seekerEmail || "—"}</strong>
                    <span style="color: #10b981; font-size: 0.68rem; font-weight:700;"> ✓ Verified</span>
                  </div>
                  <div>
                    <span style="color: var(--text-muted);">Date of Birth:</span><br>
                    <strong style="color: var(--text-secondary);">${inv.dob || "—"}</strong>
                  </div>
                  <div>
                    <span style="color: var(--text-muted);">Lineage Sponsor:</span><br>
                    <strong style="color: var(--gold-400); font-family: var(--font-mono);">${inv.sponsorCode || ((this.model && typeof this.model.getDefaultMentorCode === "function") ? this.model.getDefaultMentorCode() : "SKHM-ADM1-7788-9900")}</strong>
                  </div>
                </div>
              </div>

              <!-- ANCESTRAL LINEAGE DOSSIER -->
              <div class="dossier-box" style="background: rgba(245, 158, 11, 0.06); border: 1px solid rgba(245, 158, 11, 0.25); border-radius: 8px; padding: 0.85rem 0.95rem; margin-bottom: 0.85rem;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.55rem; font-size: 0.78rem;">
                  <div>
                    ...
                    <strong style="color: var(--gold-400); font-size: 0.83rem;">${inv.lineage?.paternalGotra || "Not specified"}</strong>
                  </div>
                  <div>
                    <span style="color: var(--text-muted);">Maternal Gotra:</span><br>
                    <strong style="color: var(--gold-400); font-size: 0.83rem;">${inv.lineage?.maternalGotra || "Not specified"}</strong>
                  </div>
                  <div>
                    <span style="color: var(--text-muted);">Kuldevi / Ishtadevata:</span><br>
                    <strong style="color: #10b981; font-size: 0.83rem;">${inv.lineage?.kuldevi || "Not specified"}</strong>
                  </div>
                  <div>
                    <span style="color: var(--text-muted);">Ancestral Village:</span><br>
                    <strong style="color: #38bdf8; font-size: 0.83rem;">${inv.lineage?.village || "Not specified"}</strong>
                  </div>
                </div>
              </div>
              ...
              <!-- KYC DOCUMENT PROOFS -->
              <div class="dossier-box" style="background: var(--bg-card, rgba(0,0,0,0.25)); border: 1px solid var(--border-subtle, rgba(255,255,255,0.08)); border-radius: 8px; padding: 0.85rem 0.95rem; margin-bottom: 0.85rem;">
                <div style="font-size: 0.72rem; font-weight: 800; color: #38bdf8; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.55rem; display: flex; align-items: center; justify-content: space-between;">
                  <span style="display: flex; align-items: center; gap: 0.35rem;">
                    <span>📄</span> KYC Document Proofs &amp; Image Data Links
                  </span>
                  <span style="font-size: 0.65rem; color: var(--text-muted);">Click card to open Lightbox</span>
                </div>
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.5rem;">
                  <div class="btn-preview-document-image" data-doc-title="Government ID Proof" data-doc-type="id_proof" data-invite-id="${inv.id}" style="background: var(--bg-primary, rgba(0,0,0,0.45)); border: 1.5px solid ${idProofAvailable ? '#10b981' : 'var(--border-subtle)'}; border-radius: 6px; padding: 0.55rem 0.4rem; text-align: center; cursor: pointer;">
                    <div style="font-size: 1.3rem; margin-bottom: 0.15rem;">📄</div>
                  ...
                  <div class="btn-preview-document-image" data-doc-title="Address Proof Document" data-doc-type="address_proof" data-invite-id="${inv.id}" style="background: var(--bg-primary, rgba(0,0,0,0.45)); border: 1.5px solid ${addrProofAvailable ? '#10b981' : 'var(--border-subtle)'}; border-radius: 6px; padding: 0.55rem 0.4rem; text-align: center; cursor: pointer;">
                    <div style="font-size: 1.3rem; margin-bottom: 0.15rem;">🏠</div>
                  ...
                  <div class="btn-preview-document-image" data-doc-title="Devotee Passport Photo" data-doc-type="photo" data-invite-id="${inv.id}" style="background: var(--bg-primary, rgba(0,0,0,0.45)); border: 1.5px solid ${photoAvailable ? '#10b981' : 'var(--border-subtle)'}; border-radius: 6px; padding: 0.55rem 0.4rem; text-align: center; cursor: pointer;">
                    <div style="font-size: 1.3rem; margin-bottom: 0.15rem;">📸</div>
              ...
              <!-- DECISION ACTIONS -->
              <div style="background: rgba(16, 185, 129, 0.08); border: 1.5px solid #10b981; border-radius: 10px; padding: 1rem;">
                <div style="font-size: 0.78rem; font-weight: 800; color: #10b981; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.65rem; display: flex; align-items: center; gap: 0.35rem;">
                ...
                ${!isApproved ? `
                  <div style="margin-bottom: 0.75rem;">
                    ...
                    <select id="drawer-select-role-${inv.id}" class="select-pairing-role" data-invite-id="${inv.id}" style="width: 100%; box-sizing: border-box; font-size: 0.82rem; padding: 0.5rem 0.65rem; background: var(--bg-primary, #000); border: 1.5px solid var(--gold-400); color: var(--text-primary); border-radius: 6px; font-weight: 700;">
                      <option value="DEVOTEE" ${defaultRole === "DEVOTEE" ? "selected" : ""}>Devotee (Level 4 — House Clean &amp; Three Diya)</option>
                      <option value="TRAINEE" ${defaultRole === "TRAINEE" ? "selected" : ""}>Trainee Sadhak (Level 3 — Initiation Practice)</option>
                      <option value="HEALER" ${defaultRole === "HEALER" ? "selected" : ""}>Certified Healer (Level 2 — Mentor Guide)</option>
                    </select>
                  </div>
                  ...
                  <div style="margin-bottom: 0.85rem;">
                    ...
                    <textarea id="drawer-mentor-notes-${inv.id}" placeholder="Enter specific instructions or document revision requests for applicant..." style="width: 100%; box-sizing: border-box; height: 50px; font-size: 0.78rem; padding: 0.45rem 0.65rem; background: var(--bg-primary, #000); border: 1px solid var(--border-subtle); border-radius: 6px; color: var(--text-primary); resize: vertical;"></textarea>
                  </div>

                  <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                    <button type="button" class="btn btn-sm btn-drawer-approve-induct" data-invite-id="${inv.id}" style="flex: 2; min-width: 130px; background: linear-gradient(135deg, #10b981, #059669); color: #fff; font-weight: 800; font-size: 0.82rem; padding: 0.6rem 0.85rem; border: none; border-radius: 6px; cursor: pointer; box-shadow: 0 4px 14px rgba(16,185,129,0.35); display: flex; align-items: center; justify-content: center; gap: 0.35rem;">
                      <span>✓</span> <span>Approve &amp; Induct to Lineage</span>
                    </button>
                    <button type="button" class="btn btn-sm btn-drawer-request-revision" data-invite-id="${inv.id}" style="flex: 1; min-width: 100px; background: rgba(245, 158, 11, 0.2); border: 1px solid #f59e0b; color: #f59e0b; font-weight: 700; font-size: 0.78rem; padding: 0.6rem 0.75rem; border-radius: 6px; cursor: pointer;">
                      📝 Revision
                    </button>
                    <button type="button" class="btn btn-sm btn-drawer-reject" data-invite-id="${inv.id}" style="background: rgba(239, 68, 68, 0.15); border: 1px solid #ef4444; color: #ef4444; font-weight: 700; font-size: 0.78rem; padding: 0.6rem 0.75rem; border-radius: 6px; cursor: pointer;">
                      ✕ Reject
                    </button>
                  </div>
                ` : `
                  <div style="background: rgba(16, 185, 129, 0.15); border: 1px solid #10b981; border-radius: 6px; padding: 0.75rem; text-align: center;">
                    ...
                    <div style="font-size: 0.75rem; color: var(--text-secondary);">
                      Applicant is officially bound to your downline lineage as <strong>${inv.assignedRole || "Devotee"}</strong>.
                    </div>
                  </div>
                `}
              </div>
...
3. Once validated, your upline mentor confirms activation to begin real-time lineage synchronization!`;
  ...
  renderAuthMatrixInSettings(matrix, currentRole = "MASTER") {
    ...
    catSelects.forEach((sel) => {
      ...
      sel.innerHTML = `<option value="ALL">All Categories (${matrix.length})</option>` +
    ...
    matrix.forEach((item) => {
      ...
      if (cat !== lastCategory) {
        ...
        html += `<tr class="auth-matrix-category-header" data-category="${cat}">
          <td colspan="6" style="padding: 0.6rem 0.75rem; background: var(--bg-hover, rgba(255,255,255,0.06)); font-weight: 700; font-size: 0.8rem; color: var(--gold-400, #f59e0b); border-bottom: 2px solid var(--border-subtle, rgba(255,255,255,0.15)); border-top: 1px solid var(--border-subtle, rgba(255,255,255,0.15));">
            📁 ${cat}
          </td>
        </tr>`;
      }

      const indent = item.level === 1 ? "&nbsp;&nbsp;&nbsp;&nbsp;↳ 📂 " : item.level === 2 ? "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;↳ ⚙️ " : "🖥️ ";
      ...
      html += `<tr data-item-id="${item.id}" data-category="${cat}" data-search-text="${(displayName + " " + item.id + " " + cat + " " + (item.type||"")).toLowerCase()}" class="auth-matrix-row">
  ...
  _initAuthMatrixSearchListeners() {
    const searchInputs = [
      document.getElementById("auth-matrix-search-input"),
      document.getElementById("auth-matrix-modal-search")
    ].filter(Boolean);
    ...
    const applyFilter = () => {
      const query = (searchInputs[0]?.value || searchInputs[1]?.value || "").toLowerCase().trim();
      const selectedCat = catSelects[0]?.value || catSelects[1]?.value || "ALL";

      const rows = document.querySelectorAll(".auth-matrix-row");
      const categoryHeaders = document.querySelectorAll(".auth-matrix-category-header");
...
ProfileView.prototype.openTierProfilesPanel = function(tierKey, allProfiles, onSelectMember, onLoadWorkspace, onOpenTree) {
  ...
  this.onLoadWorkspaceCb = onLoadWorkspace;
  this.onOpenTreeCb = onOpenTree;

  const tierMetaMap = {
    HEALER: { icon: "🌿", title: "Tier 2 • Certified Healers (Master Hub)", badge: "Healers" },
    TRAINEE: { icon: "🔥", title: "Tier 3 • Trainee Sadhak (Sadhana Core)", badge: "Trainees" },
  ...
  this.currentTierProfiles = (allProfiles || []).filter(p => {
    ...
    if (tierKey === "TRAINEE") return type === "TRAINEE" || type === "SADHAK" || lvl === 3;
    if (tierKey === "DEVOTEE") return type === "DEVOTEE" || type === "SEEKER" || lvl === 4 || lvl === 5;
    return true;
  });

  if (this.tierPanelCount) {
    this.tierPanelCount.textContent = this.currentTierProfiles.length + " Members";
  }
...
ProfileView.prototype.renderTierProfilesList = function(profiles) {
  ...
  if (!profiles || profiles.length === 0) {
    container.innerHTML = '<div style="padding: 1rem; text-align: center; color: var(--text-muted); font-size: 0.78rem;"><span>🍃</span><p style="margin: 0.25rem 0 0 0;">No member profiles found in this tier.</p></div>';
  ...
  profiles.forEach((prof, idx) => {
    ...
    item.className = "tier-member-row-item " + (idx === 0 ? "active" : "");
    item.setAttribute("data-profile-id", prof.id);
    item.innerHTML = [
      '<div class="tier-item-avatar">' + initials + '</div>',
      '<div class="tier-item-info">',
      '  <span class="tier-item-name">' + (prof.name || 'Member') + '</span>',
      '  <span class="tier-item-sub">' + (prof.referenceCode || prof.city || 'Active Member') + '</span>',
      '</div>',
      '<span class="tier-item-status-dot" title="Active Member"></span>'
    ].join("");

    item.addEventListener("click", () => {
      container.querySelectorAll(".tier-member-row-item").forEach(el => el.classList.remove("active"));
      item.classList.add("active");
  ...
  // Automatically display details of the first member
  this.renderTierProfileDetails(profiles[0]);
};
